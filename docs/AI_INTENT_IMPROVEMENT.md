...
### 1.1 普通成员（MEMBER）可用的AI功能

**活动相关：**
| 功能 | 涉及类 | 实现方式 |
|------|--------|---------|
| 申请活动报名 | `RegistrationDAO` | AI引导用户确认报名信息，调用 `RegistrationDAO.register()` |
| 查看可参加的活动 | `ActivityDAO` | AI返回活动列表摘要，引导用户进入详情页 |
| 查询报名状态 | `RegistrationDAO.findByUserId()` | AI汇总用户的报名情况 |

**项目相关：**
| 功能 | 涉及类 | 实现方式 |
|------|--------|---------|
| 申请加入项目 | `ProjectServlet` | AI收集用户意向，调用 `ProjectMemberApplication` 相关方法 |
| 查看可加入的项目 | `ProjectDAO` | AI汇总项目列表信息 |
| 查询申请状态 | `ProjectDAO.getMemberApplications()` | AI汇总用户的申请情况 |

**新闻与问题反馈：**
| 功能 | 涉及类 | 实现方式 |
|------|--------|---------|
| 提交活动新闻投稿 | `NewsServlet` | AI引导用户提供新闻内容，调用 `NewsDAO.insert()` |
| 查看新闻审核状态 | `NewsDAO.findByCreatorId()` | AI汇总用户投稿情况 |
| 提交问题反馈 | `MemberProblemServlet` | AI引导用户描述问题，调用 `ProblemReportDAO.insert()` |
| 查看反馈处理进度 | `ProblemReportDAO.findByUserId()` | AI汇总问题处理状态 |

### 1.2 管理员（ADMIN）可用的AI功能

**用户审核：**
- AI列出待审核用户，管理员确认后批量操作（`UserDAO.approve()`/`reject()` + `MemberProfileDAO.insert()`）
...
**活动与新闻审核：**
- 批量审核活动（`ActivityDAO.approveActivity()`）
- 批量审核新闻（`NewsDAO.approve()`）

### 1.3 游客（GUEST）可用的AI功能
...
```java
public String buildAdminPrompt() {
    return "你是黄山学院软件小组的AI管理员助手。\n" +
           "你拥有以下权限：\n" +
           "- 审核用户注册\n- 处理问题反馈\n- 审核活动报名\n" +
           "- 审核新闻发布\n- 一键修复常见问题\n" +
           "需要执行操作时使用 [ACTION]:action_name|param1=value1 格式。";
}
```

### 2.2 分阶段实施路线

- **第一阶段（基础）**：MEMBER操作引导，AI理解意图并跳转到对应页面，不实际执行操作（只做导航）
- **第二阶段（核心）**：管理员通过AI执行实际操作，严格权限校验 + 操作日志
- **第三阶段（高级）**：AI学习历史处理记录，自动分类处理常见问题，支持批量操作

### 2.3 安全要点

1. 每个ACTION都必须校验当前用户角色
2. 记录所有AI触发的实际操作到操作日志
3. 高风险操作（删除、批量审核）需要前端二次确认
4. AI绝对不能返回用户密码等敏感信息
